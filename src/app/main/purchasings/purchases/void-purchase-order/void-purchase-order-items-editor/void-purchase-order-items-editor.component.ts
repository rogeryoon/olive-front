...
import { OliveVoidPurchaseOrderItemDatasource } from './void-purchase-order-item-datasource';
...
@Component({
  selector: 'olive-void-purchase-order-items-editor',
  templateUrl: './void-purchase-order-items-editor.component.html',
  styleUrls: ['./void-purchase-order-items-editor.component.scss'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => OliveVoidPurchaseOrderItemsEditorComponent),
      multi: true,
    },
    {
...
export class OliveVoidPurchaseOrderItemsEditorComponent extends OliveEntityFormComponent implements ControlValueAccessor, Validator {
  displayedColumns = ['productVariantId', 'name', 'balance', 'price', 'quantityDue', 'quantity', 'remark', 'actions'];
  itemsDataSource: OliveVoidPurchaseOrderItemDatasource = new OliveVoidPurchaseOrderItemDatasource(this.cacheService);
      ...
      pItems
        .forEach((pItem: PurchaseOrder) => {
          pItem.purchaseOrderItems
            .filter((sItem: PurchaseOrderItem) => sItem.quantity > sItem.cancelQuantity && sItem.quantity > sItem.balance)
            .forEach((sItem: PurchaseOrderItem) => {
              this.itemsDataSource.addNewItem({
                quantity: sItem.balance * -1,
                balance: 0,
